...
export function Accordion({
    ...
    ${isOpen ? "rounded-[10px] primary-bg-color" : "last:border-b last:border-[#1F1F1F]"}`}>
            <button
                type="button"
                onClick={onToggle}
                className="flex w-full flex-col text-left"
            >
                <div className="pb-[5px] pr-[15px] pl-[15px] text-xs font-semibold tracking-wider text-gray-500 uppercase border-b border-[#1F1F1F] last:border">
                    STEP {stepNumber} OF {totalSteps}
                </div>
                <div className={`flex w-full items-center justify-between p-[15px] ${!isOpen ? "pb-[0px]" : ""}`}>
                    <div className="flex items-center gap-3">
                        <div className="text-gray-400">{icon}</div>
                        <h2 className="text-xl font-normal font-medium text-[22px] text-gray-900">{title}</h2>
                    </div>
